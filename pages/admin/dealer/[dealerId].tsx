...
import { ManufactorDto, UpdateDealerDto } from 'generated/api/admin';
...
export default function AdminUpdateDealer() {
  const router = useRouter();
  const dealerId = +get(router.query, 'dealerId', '0');
  ...
  >();
  const methods = useForm<UpdateDealerDto>({
    mode: 'onSubmit',
  ...
  const { data: dealer } = useGetDealerByAdmin(dealerId);

  const { mutate: updateDealerMutate } = useUpdateDealer(dealerId);

  useEffect(() => {
    if (!dealer) return;
    reset(dealer);
    setSelectedManufactor(dealer.manufactor);

    dealer.logoUrl && setSelectedImage(dealer.logoUrl);
  }, [dealer]);
            ...
            <form
              onSubmit={handleSubmit((data) => {
                ...
                handleUpload()
                  ...
                  .catch(() => console.log('제조사 등록에 실패하였습니다.'));
              })}
            >
              <Button
                text="저장하기"
...
AdminUpdateDealer.getLayout = function getLayout(page: React.ReactElement) {
